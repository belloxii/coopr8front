          ...
          <table className="w-full text-left text-sm text-foreground">
            <thead>
              <tr className="border-b bg-muted">
                {['SN', 'Name', 'Loan Type', 'Loan Amount', 'Repaid Amount', 'Date', 'Balance', 'Status'].map((header) => (
                  <th key={header} className="py-4 px-4 font-bold text-muted-foreground">{header}</th>
                ))}
              </tr>
            </thead>
            <tbody>
                ...
                filteredRepayments.map((item, index) => {
                  ...
                  return (
                    <tr key={index} className="border-b last:border-none">
                      <td className="py-3 px-4 font-semibold text-muted-foreground">{index + 1}</td>
                      <td>
                        {item?.user ? (
                          <Link
                      ...
                      <td className="py-3 px-4 font-semibold text-muted-foreground">
                        ...
                        {parentLoan?.id && (
                          <button
                            onClick={() => navigate(`/admin/loans/${parentLoan.id}`)}
                            className="text-blue-600 text-xs border border-blue-500 px-2 py-[1px] ml-1 rounded-full hover:bg-blue-100 transition">
                            View
                          </button>
                        )}
                      </td>
